...
public class HistoryCommand : AsyncCommand<HistoryCommand.Settings>
{
    ...
    {
        var service = await ServiceRegistry.FindAsync(settings.Service);
        if (service is null)
        {
        ...
        return settings.Id is null
            ? await ListAsync(service, settings)
            : await InspectAsync(service, settings);
    }

    // ── List mode ─────────────────────────────────────────────────────────────

    private static async Task<int> ListAsync(ServiceEntry service, Settings settings)
    {
        var entries = await HistoryService.LoadAllAsync(settings.Service);
        ...
        AnsiConsole.MarkupLine($"[cyan]{settings.Service}[/] [grey]— {countLabel} requests[/]");
        OutputHelpers.Separator(lineWidth);
        AnsiConsole.MarkupLine(
            $"  [grey]{"#".PadRight(idWidth)}  {"Method".PadRight(mthWidth)}  {"Path".PadRight(pathWidth)}  " +
            $"{"Status".PadRight(statusWidth)}  {"Duration".PadRight(durWidth)}  {"Operation".PadRight(opWidth)}  When[/]");
        OutputHelpers.Separator(lineWidth);
        ...
        foreach (var e in recent)
        {
            ...
            AnsiConsole.MarkupLine(
                $"  [grey]{id}[/]  [white]{method}[/]  [grey]{path}[/]  " +
                $"[{statusColor}]{status}[/]  [grey]{duration}  {opId}  {when}[/]");
        }
    ...
    private static async Task<int> InspectAsync(ServiceEntry service, Settings settings)
    {
        var e = await HistoryService.FindAsync(settings.Service, settings.Id!.Value);
        if (e is null)
        {
        ...
        if (!settings.ResponseOnly)
        {
            ...
            OutputHelpers.Separator(sepWidth);
            AnsiConsole.MarkupLine($"  {e.Method} {e.Url}");
